...
This walks through installing and running the `effigies` CLI — the authoring and orchestration
layer that sits above the `incognito` anonymisation engine. If you want to work directly with the
`alterego` or `incognito` libraries instead, more docs are coming; for now, see their READMEs in
the [monorepo](https://github.com/identigon/identigon).
...
- A clone of [identigon/identigon](https://github.com/identigon/identigon), built with
  `./gradlew build` — this produces a single runnable jar at `effigies/build/libs/effigies.jar`

## 1. Discover the schema

**Why:** before anything is generated, it's worth seeing exactly what `effigies` will be working
with — every table and column it can see, and how they relate. It's a quick sanity check, and it
touches metadata only; no row values are ever read at this step.

```sh
export IDENTIGON_SOURCE_PASSWORD="secret"
java -jar build/libs/effigies.jar discover \
  --source-url "jdbc:postgresql://..." --source-user "admin"
```

**Produces:** a human-readable summary printed to stdout — nothing is written to a file. Each
table, with every column annotated by type and, where relevant, `pk` / `fk -> <table>`:
...
ending in `email` suggests `DIRECT_ID`) and leaves the suggestion as a comment — never applied —
so you have a head start without anything being silently classified on your behalf.

```sh
java -jar build/libs/effigies.jar scaffold \
  --source-url "jdbc:postgresql://..." --source-user "admin" \
  --out ./policy.draft.yaml
```

**Produces:** a `policy.draft.yaml` file, one entry per table/column, every `role:` left blank —
a suggestion in the comment where a heuristic matched, a pointer to the role vocabulary where none
did:
...
and a foreign key — the heuristics only look at column *names*, not constraints. You still have to
...
*is*. It's deliberately a human decision (optionally assisted): the run refuses to start until
every column has an explicit role, so nothing new in the source can slip through unclassified.

Effigies ships an Agent Skill (`.agents/skills/identigon-policy-author/SKILL.md`) that AI
...
Effigies never assigns a role behind your back. You can also hand-edit the YAML directly; it's a
plain declarative file.

**Produces:** the same file, every `role:` now filled in and renamed to `policy.yaml` — a
runnable config with nothing left blank:
...
**Why:** this is the actual anonymisation — a deterministic, model-free `incognito` execution
against the now-finished policy. Nothing judgment-shaped happens here; if the policy is valid, the
...
**Produces:** the anonymised clone, loaded into the target database — plus a short summary and
three DPIA report files written to the working directory:
...
transformation applied per column, any misdeclaration-lint or structural-uniqueness findings, and
a handful of illustrative synthetic sample rows, so nobody has to trust the run blind.
